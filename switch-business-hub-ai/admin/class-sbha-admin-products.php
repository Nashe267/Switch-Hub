...
class SBHA_Admin_Products {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_sbha_save_product_image', array($this, 'ajax_save_product_image'));
    }
    
    public function ajax_save_product_image() {
        ...
        if (empty($product_key)) wp_send_json_error('No product specified');
        
        $images = get_option('sbha_product_images', array());
        if ($image_url) {
            $images[$product_key] = $image_url;
        } else {
            unset($images[$product_key]);
        }
        update_option('sbha_product_images', $images);
        
        wp_send_json_success(array('message' => 'Image saved!'));
    }
    ...
    public function render() {
        ...
        <div class="wrap sbha-admin-wrap">
                    ...
                    <?php elseif ($edit_product): ?>
                        <form method="post">
                            ...
                            <div class="sbha-form-row">
                                ...
                                $current_img = $product_images[$editing] ?? ($edit_product['image'] ?? '');
                                ?>
                                <div style="display:flex;gap:12px;align-items:flex-start">
                                    <div id="productImagePreview" style="width:120px;height:120px;background:#f0f0f0;border-radius:8px;display:flex;align-items:center;justify-content:center;overflow:hidden;border:2px dashed #ccc">
                                        <?php if ($current_img): ?>
                                        <img src="<?php echo esc_url($current_img); ?>" style="width:100%;height:100%;object-fit:cover">
                                        <?php else: ?>
                                        <span style="color:#999;font-size:30px">📷</span>
                                        <?php endif; ?>
                                    </div>
                                    <div>
                                        <input type="hidden" name="product_image" id="productImageUrl" value="<?php echo esc_url($current_img); ?>">
                                        <button type="button" class="sbha-btn sbha-btn-outline" onclick="sbhaSelectProductImage()" style="margin-bottom:8px">📷 Choose Image</button>
                                        <?php if ($current_img): ?>
                                        <br><button type="button" class="sbha-btn" style="color:#dc2626;background:none;border:none;font-size:12px;cursor:pointer" onclick="sbhaRemoveProductImage()">🗑️ Remove</button>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </div>
            ...
            function sbhaSelectProductImage() {
                var frame = wp.media({
                ...
                frame.on('select', function() {
                    ...
                    document.getElementById('productImageUrl').value = attachment.url;
                    document.getElementById('productImagePreview').innerHTML = '<img src="' + attachment.url + '" style="width:100%;height:100%;object-fit:cover">';
                });
                frame.open();
            }
            
            function sbhaRemoveProductImage() {
                document.getElementById('productImageUrl').value = '';
                document.getElementById('productImagePreview').innerHTML = '<span style="color:#999;font-size:30px">📷</span>';
            }
            </script>
        </div>
        <?php
    ...
    private function save_product($data) {
        ...
        // Save product image
        $image_url = esc_url_raw($data['product_image'] ?? '');
        $images = get_option('sbha_product_images', array());
        if ($image_url) {

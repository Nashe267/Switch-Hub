        ...
        <div class="wrap sbha-admin-wrap">
            ...
            <div class="sbha-order-detail">
                <div>
                    ...
                    <div class="sbha-card">
                        ...
                        <table class="sbha-items-table">
                            ...
                            <?php foreach ($items as $item): 
                                $item_qty = intval($item['quantity'] ?? $item['qty'] ?? 1);
                                $item_price = floatval($item['price'] ?? $item['unit_price'] ?? 0);
                                $item_design_fee = floatval($item['design_fee'] ?? 0);
                                $item_delivery_fee = floatval($item['delivery_fee'] ?? 0);
                                $item_total = floatval($item['subtotal'] ?? (($item_price * $item_qty) + $item_design_fee + $item_delivery_fee));
                            ?>
                            <tr>
                                <td>
                                    <strong><?php echo esc_html($item['name'] ?? $item['product_name'] ?? $item['product'] ?? ''); ?></strong>
                                    <br><small style="color:#666"><?php echo esc_html($item['variation'] ?? $item['variant_name'] ?? $item['variant'] ?? ''); ?></small>
                                    <?php if (!empty($item['sku']) || !empty($item['variant_sku'])): ?>
                                    <br><small style="color:#9ca3af">SKU: <?php echo esc_html($item['sku'] ?? $item['variant_sku']); ?></small>
                                    <?php endif; ?>
                                    <?php if (!empty($item['brief'])): ?>
                                    <br><small style="color:#059669">📝 <?php echo esc_html(substr($item['brief'], 0, 100)); ?></small>
                                    <?php endif; ?>
                                    <?php if ($item_design_fee > 0): ?>
                                    <br><small style="color:#059669">🎨 Design fee: R<?php echo number_format($item_design_fee, 2); ?></small>
                                    <?php endif; ?>
                                    <?php if ($item_delivery_fee > 0): ?>
                                    <br><small style="color:#2563eb">🚚 Delivery fee: R<?php echo number_format($item_delivery_fee, 2); ?></small>
                                    <?php endif; ?>
                                </td>
                                <td><?php echo esc_html($item_qty); ?></td>
                    ...
                    <?php if (!empty($single->chat_transcript)): ?>
                    <div class="sbha-card">
                        <h3>🧠 AI Conversation Transcript</h3>
                        <div style="background:#111827;color:#f3f4f6;padding:14px;border-radius:10px;max-height:320px;overflow:auto;white-space:pre-wrap;font-size:12px;line-height:1.5"><?php echo esc_html($single->chat_transcript); ?></div>
                    </div>
                    <?php endif; ?>
